...
test('gathers wood, builds a garden, and lets the player assign a grower', async ({ page }) => {
  ...
  await page.waitForFunction(() => window.__FRONTIER__?.diagnostics().nearestNode?.kind === 'tree')
  await page.keyboard.up('w')
  ...
  if (!(await diagnostics(page)).placementValid) {
    await page.keyboard.down('d')
    await page.waitForFunction(() => window.__FRONTIER__?.diagnostics().placementValid === true)
